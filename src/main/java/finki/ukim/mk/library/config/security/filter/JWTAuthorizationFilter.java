...
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        String header = request.getHeader(JWTAuthConsts.HEADER_STRING);
        if (header == null || !header.startsWith(JWTAuthConsts.TOKEN_PREFIX)){
            chain.doFilter(request,response);
            return;
        }

        String user = JWT.require(Algorithm.HMAC256(JWTAuthConsts.SECRET.getBytes()))
                .build()
                .verify(header.replace(JWTAuthConsts.TOKEN_PREFIX, ""))
                .getSubject();
